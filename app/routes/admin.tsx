...
export default function Admin() {
    ...
    const selfID = useRef<SelfID<ModelTypes> | undefined>();
    ...
    useEffect(() => {
        ...
        const connect = async () => {
            if (typeof document !== 'undefined') {
                ...
                //@ts-ignore
                const authProvider = new module.default.EthereumAuthProvider(window.ethereum, window.ethereum.selectedAddress)
                ...
                const profileData = await self.get("basicProfile");
                setProfile(profileData);

                //blogPostTile.current = await self.client.tileLoader.load()

                const blogPostDefinitionID = await self.client.dataModel.getDefinitionID('blogPosts');
                const doc = await self.client.dataStore.getRecordDocument(blogPostDefinitionID as string);
    ...
    const publish = async () => {
        if (typeof document !== 'undefined') {
            ...
            const doc = await selfID.current?.client.dataModel.createTile('BlogPost', {
                date: new Date().toISOString(),
                text: text,
              })
            if (doc) {
                selfID.current?.set('blogPosts', { blogPosts: [...blogPosts,{ id: doc.id.toUrl(), title: title }] })
            }
